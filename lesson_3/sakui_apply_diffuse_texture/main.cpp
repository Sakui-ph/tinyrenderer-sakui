...
{   
    ...
    for (int y = bboxmin.y; y < bboxmax.y; y++)
    {
        for (int x = bboxmin.x; x < bboxmax.x; x++)
        {
            ...
            {
                // to apply the texture, we need to change this image set to the texture color
                image.set(x, y, color);
...
void fill_triangle_barycentric_z_texture(Vec3i t0, Vec3i t1, Vec3i t2, TGAImage &image, TGAImage texture, float *z_buffer)
{   
    if (t0.y==t1.y && t0.y==t2.y) return;
    // First, we want to get the bounding boxes of the triangle
    Vec2i bboxmax(0, 0);
    ...
    for (int y = bboxmin.y; y < bboxmax.y; y++)
    {
        for (int x = bboxmin.x; x < bboxmax.x; x++)
        {
            ...
            {
                // to apply the texture, we need to change this image set to the texture color
                TGAColor color = texture.get(x * texture.get_width(), y * texture.get_height());
                image.set(x, y, color);
                z_buffer[z_idx] = z_value;
            }
        }
